...
public class ReefscapeAnalyzer extends Analyzer<UnknownScoreBreakdown, ReefscapeData> {
    ...
    @Override
    protected ReefscapeData
            computeData(Analyzer<UnknownScoreBreakdown, ReefscapeData>.Data inputs) {
        return new ReefscapeData(
                ...
                mostCommon(map(extractData(inputs.getPitEntries(), "/specs/drivetrain",
                        DataEntry::getInteger), DrivetrainType::of)),
                mostCommon(extractData(inputs.getPitEntries(), "/specs/weight",
                        DataEntry::getInteger)),
                mostCommon(
                        extractData(inputs.getPitEntries(), "/specs/size", DataEntry::getInteger)),
                ...
                // "/general/defense",
                // DataEntry::getInteger, Analyzer::average)),
                countDistinct(map(extractMergeData(inputs.getMatchEntries(), "/auto/start_pos",
                        DataEntry::getInteger, Analyzer::mostCommon), StartPosition::of)),
                mostCommon(
                        extractData(inputs.getPitEntries(), "/auto/score", DataEntry::getInteger)),
                countDistinct(map(
                        extractMergeData(inputs.getMatchEntries(), "/auto/scoring",
                                DataEntry::getIntegers, Analyzer::mostCommon).stream()
                                                                             .flatMap(List::stream)
                                                                             .toList(),
                        AutoAction::of)),
                nullableMap(List.of(nullableMapEntry(CoralLevel.L1,
                        ...
                        nullableMapEntry(CoralLevel.L2,
                                average(extractMergeData(inputs.getMatchEntries(),
                                        "/teleop/coral_l2", DataEntry::getInteger,
                                        Analyzer::mostCommon))),
                        nullableMapEntry(CoralLevel.L3,
                                average(extractMergeData(inputs.getMatchEntries(),
                                        "/teleop/coral_l3", DataEntry::getInteger,
                                        Analyzer::mostCommon))),
                        nullableMapEntry(CoralLevel.L4,
                                average(extractMergeData(inputs.getMatchEntries(),
                                        "/teleop/coral_l4", DataEntry::getInteger,
                                        Analyzer::mostCommon))))),
                nullableMap(List.of(
                        nullableMapEntry(AlgaeScore.PROCESSOR,
                                average(extractMergeData(inputs.getMatchEntries(),
                                        "/teleop/algae_processor", DataEntry::getInteger,
                                        Analyzer::mostCommon))),
                        nullableMapEntry(AlgaeScore.BARGE_NET,
                                average(extractMergeData(inputs.getMatchEntries(),
                                        "/teleop/algae_net", DataEntry::getInteger,
                                        Analyzer::mostCommon))))),
                countDistinct(map(
                        extractMergeData(inputs.getMatchEntries(), "/teleop/pickups",
                                DataEntry::getIntegers, Analyzer::mostCommon).stream()
                                                                             .flatMap(List::stream)
                                                                             .toList(),
                        GamePiece::of)),
                countDistinct(map(extractMergeData(inputs.getMatchEntries(), "/endgame/status",
                ...
                average(map(extractMergeData(inputs.getMatchEntries(), "/summary/damage",
                        DataEntry::getBoolean, Analyzer::mostCommon), b -> b ? 1 : 0)),
                countDistinct(map(extractMergeData(inputs.getMatchEntries(), "/summary/fouls",
                        DataEntry::getIntegers, Analyzer::mostCommon).stream()
                                                                     .flatMap(List::stream)
                                                                     .toList(),
                        Fouls::of)),
                countDistinct(map(extractMergeData(inputs.getMatchEntries(), "/summary/disabled",
        ...
        return List.of(
                ...
                new StatisticsPage("Auto",
                        List.of(new PieChartStatistic("Start Position", data.autoStartPositions()),
                                new StringStatistic("Expected Score",
                                        data.autoPitAnticipatedScore()),
    ...
    private static RadarStatistic driveTeamRadar(ReefscapeData data) {
        return new RadarStatistic("Drive Team", 5,
                nullableMap(List.of(
                        nullableMapEntry("Communication", data.driveTeamCommunication()),
